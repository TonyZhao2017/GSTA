...
from models.AFT2 import AFTFull
...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.his_encoder = LstmEncoder(embed_dim=embed_dim)
        self.am_encoder = AgentMapEncoder(node_dim=node_dim,
                                          edge_dim=edge_dim,
                                          embed_dim=embed_dim,
                                          num_heads=num_heads,
                                          dropout=dropout)
        self.as_encoder = AgentSpaceEncoder(historical_steps=historical_steps,
                                            node_dim=node_dim,
                                            edge_dim=edge_dim,
                                            embed_dim=embed_dim,
                                            num_heads=num_heads,
        ...
        self.al_enhance = AgentMapEncoder(node_dim=node_dim,
                                          edge_dim=edge_dim,
                                          embed_dim=embed_dim,
                                          num_heads=num_heads,
                                          dropout=dropout)
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        # start lstm encoder
        # [758, 20, 64], [758, 64]
        h = self.his_encoder(data['x'], data['bos_mask'], data['rotate_mat'])
        edge_index, edge_attr = self.drop_edge(data['lane_actor_index'], data['lane_actor_vectors'])
        am_encode = self.am_encoder(x=(data['lane_vectors'], h), edge_index=edge_index, edge_attr=edge_attr,
        ...
        if self.parallel:
            ...
            out = self.as_encoder(x=batch.x, t=None, edge_index=batch.edge_index, edge_attr=batch.edge_attr,
            ...
            for t in range(self.historical_steps):
                edge_index, edge_attr = self.drop_edge(data[f'edge_index_{t}'], data[f'edge_attr_{t}'])
                out[t] = self.as_encoder(x=data.x[:, t], t=t, edge_index=edge_index, edge_attr=edge_attr,
        ...
        out = self.al_enhance(x=(data['lane_vectors'], out), edge_index=edge_index, edge_attr=edge_attr,
                             is_intersections=data['is_intersections'], turn_directions=data['turn_directions'],
                             traffic_controls=data['traffic_controls'], rotate_mat=data['rotate_mat'])
        return out
...
class LstmEncoder(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(32, embed_dim)
        ...
        self.mlp = nn.Sequential(
            ...
            nn.Dropout(dropout),
            nn.Linear(64, 2))
        self.liner1 = nn.Linear(2, 32)
        self.liner2 = nn.Linear(embed_dim, embed_dim)
        self.liner3 = nn.Linear(embed_dim, embed_dim)

    def forward(self,
                x: torch.Tensor,
                bos_mask: torch.Tensor,
                rotate_mat: Optional[torch.Tensor]) -> torch.Tensor:
        x = torch.bmm(x, rotate_mat)
        bos_uns = bos_mask.unsqueeze(-1).float()
        bos_cat = torch.cat([x, bos_uns], dim=-1)
        stuff = self.mlp(bos_cat)
        fill = torch.where(bos_mask.unsqueeze(-1), stuff, x)
        emb = self.liner1(fill)
        _, (h, _) = self.lstm(self.activate(emb).permute(1, 0, 2))
        h = h.view(h.shape[1], h.shape[2])
        return self.activate(self.liner2(h))
...
class Gate(nn.Module):
    ...
    def __init__(self,
        ...
        self.line1 = nn.Linear(input_dim, input_dim)
        self.line2 = nn.Linear(input_dim, input_dim)
        self.lin_self = nn.Linear(input_dim, input_dim)
    ...
    def forward(self,
        ...
        if input1 is None:
           ...
           gate = torch.sigmoid(self.line1(input1) + self.line2(input2))
           return input1 + gate * (self.lin_self(input2) - input1)


class AgentSpaceEncoder(MessagePassing):
    ...
    def __init__(self,
                 ...
                 parallel: bool = False,
                 **kwargs) -> None:
        super(AgentSpaceEncoder, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.historical_steps = historical_steps
        ...
        self.center_gate = Gate(embed_dim)
        self.nbr_gate = Gate(embed_dim)
    ...
    def forward(self,
        ...
        if self.parallel:
            ...
            center_embed = self.center_gate(center_embed, his_emb)
        center_embed = center_embed + self._mha_block(self.norm1(center_embed), x, edge_index, edge_attr, his_emb,
                                                      rotate_mat,
                                                      size)
    ...
    def message(self,
        ...
        if rotate_mat is None:
            ...
            if self.parallel:
                ...
                center_his_emb = his_emb[edge_index[1]]
            ...
            nbr_embed = self.nbr_gate(nbr_embed, center_his_emb)
    ...
    def _mha_block(self,
        ...
        center_embed = self.out_proj(self.propagate(edge_index=edge_index, x=x, center_embed=center_embed,
                                                    edge_attr=edge_attr, his_emb=his_emb, rotate_mat=rotate_mat,
                                                    size=size))
...
class TemporalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        aft_encoder_layer = TemporalEncoderLayer(embed_dim=embed_dim, num_heads=num_heads, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer=aft_encoder_layer, num_layers=num_layers,
...
class TemporalEncoderLayer(nn.Module):
    ...
    def _sa_block(self,
        ...
        x = x.permute(1, 0, 2)
        x = self.atf_attn(x)
        return self.dropout1(x).permute(1, 0, 2)
...
class AgentMapEncoder(MessagePassing):
    ...
    def __init__(self,
                 ...
                 dropout: float = 0.1,
                 **kwargs) -> None:
        super(AgentMapEncoder, self).__init__(aggr='add', node_dim=0, **kwargs)
        self.embed_dim = embed_dim
        self.num_heads = num_heads
